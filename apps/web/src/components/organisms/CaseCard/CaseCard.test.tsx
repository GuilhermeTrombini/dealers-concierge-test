...
describe('CaseCard', () => {
  ...
  it('applies correct priority colors', () => {
    const { rerender } = renderWithRouter(<CaseCard case_={{ ...mockCase, priority: 1 }} />)
    expect(screen.getByText('1')).toHaveClass('text-green-600')

    rerender(<BrowserRouter><CaseCard case_={{ ...mockCase, priority: 4 }} /></BrowserRouter>)
    expect(screen.getByText('4')).toHaveClass('text-red-600')
  })

  it('displays correct status for different case statuses', () => {
    const { rerender } = renderWithRouter(<CaseCard case_={{ ...mockCase, status: 'IN_PROGRESS' }} />)
    expect(screen.getByText('IN PROGRESS')).toBeInTheDocument()

    rerender(<BrowserRouter><CaseCard case_={{ ...mockCase, status: 'RESOLVED' }} /></BrowserRouter>)
