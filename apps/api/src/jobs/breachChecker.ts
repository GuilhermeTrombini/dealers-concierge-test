...
export const breachQueue = new Queue('breach-checker', { connection })
  ...
  const worker = new Worker(
    ...
    async (_job) => {
      ...
      const breachPromises = candidates.map(async (case_) => {
        const isBreached = caseService.isCaseBreached(case_.createdAt, case_.slaMinutes)
        ...
        if (isBreached) {
          ...
          try {
            ...
            console.error(`Failed to mark case ${case_.id} as breached:`, error)
            return { caseId: case_.id, breached: false, error: (error as Error).message }
          }
        }
